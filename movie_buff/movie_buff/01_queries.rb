...
def biggest_cast
  ...
  Movie.select('movies.id, movies.title').joins(:castings).group('movies.id').order('count(castings.actor_id) desc').limit(3)
end
...
def directed_by_one_of(them)
  ...
  Movie.select(:id, :title).joins(:director).where("actors.name in ('George Lucas', 'Steven Spielberg')")
end
